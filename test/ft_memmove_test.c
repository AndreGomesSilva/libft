...
TEST (ft_memmove_test, should_return_dest_when_pass_size_0)
{
	...
	TEST_ASSERT_EQUAL(memmove(src, src+4, 0), ft_memmove(src, src+4, 0));
}
...
TEST (ft_memmove_test, overlap_when_src_is_biger_then_dest)
{
	...
	TEST_ASSERT_EQUAL(memmove(src+2, src, 4), ft_memmove(src+2, src, 4));
}
...
TEST (ft_memmove_test, overlap_whe_dest_is_biger_the_src) 
{
	...
	TEST_ASSERT_EQUAL(memmove(src, src+2, 4), ft_memmove(src, src+2, 4));
}
